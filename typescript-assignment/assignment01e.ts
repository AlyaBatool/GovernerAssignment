...
function show_magicians(){
     let original_array=['denny','lana','kale','olivia','hadid'];
     for(let a=0; a<original_array.length; a++){
...
function make_great(){
    let copied_array=['denny','lana','kale','olivia','hadid'];
    for(let r=0; r<copied_array.length; r++){
   console.log('The Great '+copied_array[r]);
    }
}
show_magicians(); //calling the original array
let new_array=make_great();
...
let first_album=(make_album('Michael Jackson','One Way Ticket','6'));
let second_album=(make_album('Timberland','The way I are','2'));
let third_album=(make_album('BTS','Blood,sweat and tears','11'));
const all_album=[first_album,second_album,third_album];
