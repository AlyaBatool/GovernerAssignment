...
const favorite_fruits=['Apple','Pear','Kiwi'];
...
const current_users=['rose','thomas','jisoo','jordan','ali','khloe'];
const new_users=['ali','sam','jessica','rose','andrew','rozi'];
for (let k=0; k<new_users.length; k++) {
    let duplicate_name=false;

    for (let l=0; l<current_users.length; l++) {
...
let pizza=['Afghani pizza','Chicken Tikka pizza','Fajita pizza'];
for(i=0; i<3; i++){
